...
if ($transport->xpdo) {
    ...
    switch ($options[xPDOTransport::PACKAGE_ACTION]) {
        ...
        case xPDOTransport::ACTION_UPGRADE:
            $modx->addPackage('msinformuser', MODX_CORE_PATH . 'components/msinformuser/model/');

            // Добавить колонку im_count
            $cusFields = array();
            $table = $modx->config['table_prefix'] . 'site_content';
            $t = $modx->prepare("SHOW COLUMNS IN {$table}");
            $t->execute();
            while ($cus = $t->fetch(PDO::FETCH_ASSOC)) {
                $cusFields[$cus['Field']] = $cus['Field'];
            }
            if (!in_array('iu_count', $cusFields)) {
                $sql = 'ALTER TABLE ' . $table . "  ADD `iu_count` INT(10) NOT NULL DEFAULT 0 AFTER `show_in_tree`;";
                $modx->exec($sql);
            }
            if (!in_array('iu_email', $cusFields)) {
                $sql = 'ALTER TABLE ' . $table . "  ADD `iu_email` VARCHAR(100) NOT NULL DEFAULT '' AFTER `iu_count`";
                $modx->exec($sql);
            }
            if (!in_array('iu_control_date', $cusFields)) {
                $sql = 'ALTER TABLE ' . $table . "  ADD `iu_control_date` INT(20) NOT NULL DEFAULT 0 AFTER `iu_email`";
                $modx->exec($sql);
            }

            $manager = $modx->getManager();
            $objects = [];
            $schemaFile = MODX_CORE_PATH . 'components/msinformuser/model/schema/msinformuser.mysql.schema.xml';
